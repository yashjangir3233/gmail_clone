...
export async function sendEmail(request, response, next) {
  try {
    ...
    const recipientAccount = await Account.findOne({ email: request.body.to });
    console.log(recipientAccount)
    if (recipientAccount) {
      recipientAccount.mailbox.inbox.push(savedEmailOut._id);
      await recipientAccount.save();
      console.log('Email added to recipient inbox', recipientAccount.email);
    } else {
      console.log('Recipient not found:', request.body.to);
...
export async function toggleEmailProperty(request, response, next) {
  try {
    ...
    console.log('seedha error gya',error);
    response.status(500);
  }
}
